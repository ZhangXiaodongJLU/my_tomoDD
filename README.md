...
------
### atoangle.c
此子函数用于将时间统一划归以小时(h)为单位,也用于将射线角度划归成以度(°)为单位.
#### 函数说明
	函数中的*p代表的是射线参数,描述的是地震射线入射时与界面所成的角度.
	变量名称及类型				变量的意义
	DOUBLE d			     地震射线入射角度中的"°"
	DOUBLE m			     地震射线入射角度中的"’"
	DOUBLE s			     地震射线入射角度中的"”"
	...
	该函数属于后期处理函数.
### atoangle_.c
此子函数也是用于将时间和射线角度的单位进行统一,并转换为F77程序中的数据类型.
#### 函数说明
	调用atoangle_.c的结果,并将结果输出到可处理的数组q中
	函数中的*p代表的是射线参数,描述的是地震射线入射时与界面所成的角度.
	该函数属于后期处理函数.
### chtof.c
该函数用于确定地震射线的反射平面,并将射线参数从字符串模式输出到浮点型数据模式。
### cluster_tomoDD.f

### compat.h
此子函数用于ANSI C语言下的宏定义的头文件
#### 函数说明
	_STDC_ 标准化(ANSI) C语言下的宏预处理命令
	_STDC_=1,表示该命令是ANSI C语言标准格式执行;
	_STDC_=0,表示该命令不是ANSI C语言格式执行;
	变量名称及宏定义           变量及宏定义的含义
	_STDC_=1:
	GENPTR			 定义一个指针变量,无类型
	PARMS(x)		 替换成x
	_STDC_=0:		 
	FENPTR			 定义一个字符串类型的指针变量
	PARMS(x)		 替换成()
注:#define const 无实际的含义
### covar.f

### datetime_.c
此函数的作用给出以字符串类型表示的现在的日期和时间
#### 函数说明
	宏定义说明			     宏的作用
	MIN(a,b) ((a)<(b)?(a):(b))	得到a与b两者间的最小值,并输出
...
### f77types.h 头文件
此函数用于F77编译器读取ANSI C语言数据类型时的一些作用函数
#### 函数说明
	  函数名		函数作用
	  F77INT	  默认整数数据类型
	  F77LOG	  默认逻辑型数据类型
